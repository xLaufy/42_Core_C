...
char *str1; and char *str2;: Pointers to characters used for comparing the contents of two memory areas.
size_t i;: A variable to iterate through bytes in the loop.

Pointer Assignment:

str1 = (char *)dest;
and	str2 = (char *)src;: Casting generic pointers to char pointers allows treating the data as
...
if (i > n) return (0);: Checks if i (loop counter) is greater than n (the number of bytes to compare).
 If true, it returns 0.

Byte Comparison in a Loop:

while (i < n) { ... }: The loop iterates for each byte to compare (up to the specified n).
